# -*- coding: utf-8 -*-
"""
Created on Wed Dec  6 21:14:38 2023
...
def calculate_total_distance(path):
    ...
    for i in range(len(path) - 1):
        distance = m.dist(path[i], path[i + 1])
        total_distance += distance
        #print(f"Distance from waypoint {i + 1} to waypoint {i + 2}: {distance}")
...
class Agent:
    ...
    def find_best_task(self, tasks, obs_positions):
        ...
        for i, task in enumerate(tasks):  # search over all tasks
            if task.active:  # don't do tasks that are done
                ...
                # Call A* algorithm from tspastar module
                print("self.x",self.x,"self.y",self.y,"task.x", int(task.x),"task.y", int(task.y))
                path = multi_tsp_astar.Astar((int(self.x), int(self.y)), (int(task.x), int(task.y)), obs_positions)
                total_distance = calculate_total_distance(path)
                #print("path",path)
                # cost = len(total_distance) / self.speed  # cost here is time
                cost = total_distance
    ...
    def update_bid(self, task_bid, agent_index):  # remove old bids and update with new bid
        for bid in self.team_bids:
            # remove old bids by the same agent if they exist
...
class World:  # holds all tasks
    ...
    def work_on_task(self, agent):  # complete task if agent is there
        ...
        if dist < agent.speed:  # is the agent actually at the task they're claiming...
...
for ts in range(0, N_time_steps):  # iterate time
    ...
    for agent in agents:  # go through each agent
